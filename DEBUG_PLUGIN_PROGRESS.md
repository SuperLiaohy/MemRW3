...
- `DebugSnapshot` 增加最近一次实际步进方式；DebugBar 状态行显示“硬件断点加速”或
  `SingleStep`，硬件 comparator 不可用并回退时显示后者。
- 源码编辑器改用固定高度虚拟行，仅渲染可见源码和行内汇编；可执行源码行建立路径／
  行号索引，源码缓存使用共享不可变行数组，滚动时不再复制或遍历整份大文件。
- 新增步进状态标签及源码／行内汇编虚拟行映射测试。

### 源码步进策略优化

- Step Over 在执行到 ELF 与目标内存字节一致的 ARM `BL/BLX/BLR` 调用时，恢复原
  PRIMASK 并使用临时硬件断点运行到返回地址；无空闲 comparator 时自动回退硬件单步。
- Step Out 优先按 `probe-rs-debug` 的 unwind PC 规则还原真实返回地址并加速；内联帧、
  ELF 范围外地址或无法安装断点时回退逐指令判断。
- Fault、watchpoint、软件断点、外部暂停和非预期硬件断点会立即结束步进；失败或取消后
  重新读取实际 Core 状态、PC、栈和 locals，并递增 `stop_id`，不再保留过期快照。
- 全局停止、断开和应用退出加入独立步进中断优先级；临时断点、PRIMASK 与用户断点均在
  返回 UI 前清理或恢复。源码步进增加 10 秒与 16384 条指令的双重上限。
- 新增调用返回地址、外部停止原因、无效源码位置和取消优先级测试。全量 release 测试：
  96 passed，0 failed，3 ignored；硬件加速路径待通过 `MEMRW3_HARDWARE_CONFIG` 板测。
