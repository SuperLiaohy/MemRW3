...
Control Bar 在未连接时保持中性底色，连接后暂停使用 warning 色，采集中使用 success 色；状态同时作用于整条 bar 的底色/描边和开始/暂停按钮，并由当前主题 palette 派生。应用默认浅色；主题按钮在深色、浅色、跟随系统三态间切换。深浅两套自定义 Style 均预先安装；Linux 后台监视 GNOME `color-scheme`/GTK theme 并更新 egui fallback，补足 winit 在 Linux 返回未知系统主题的问题。状态灯使用固定尺寸绘制图形，运行/暂停切换不会改变工具栏高度。
